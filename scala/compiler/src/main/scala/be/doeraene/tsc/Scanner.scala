...
object Scanner {
  ...
  val textToToken: Map<SyntaxKind> = {
    ...
    "def": SyntaxKind.FunctionKeyword,
    ...
    "package": SyntaxKind.NamespaceKeyword,
  ...
  */
  ...
  /* @internal */ def isUnicodeIdentifierStart(code: Int, languageVersion: ScriptTarget) {
  ...
  def makeReverseMap(source: Map<Int>): String[] {
    val result: String[] = []
    for (val name in source) {
  ...
  /* @internal */
  def computeLineStarts(text: String): Int[] {
    val result: Int[] = new Array()
    var pos = 0
    var lineStart = 0
  ...
  /* @internal */
  def computePositionOfLineAndCharacter(lineStarts: Int[], line: Int, character: Int): Int {
    Debug.assert(line >= 0 && line < lineStarts.length)
    return lineStarts[line] + character
  }

  /* @internal */
  def getLineStarts(sourceFile: SourceFile): Int[] {
   ...
   */
  def computeLineAndCharacterOfPosition(lineStarts: Int[], position: Int) {
   ...
   */
  def getCommentRanges(text: String, pos: Int, trailing: Boolean): CommentRange[] {
      ...
      switch (ch) {
        ...
        case CharacterCodes.slash:
          ...
          if (nextChar == CharacterCodes.slash || nextChar == CharacterCodes.asterisk) {
            val kind = nextChar == CharacterCodes.slash ? SyntaxKind.SingleLineCommentTrivia : SyntaxKind.MultiLineCommentTrivia
            val startPos = pos
            pos += 2
  ...
  def getLeadingCommentRanges(text: String, pos: Int): CommentRange[] {
    return getCommentRanges(text, pos, /*trailing*/ false)
  }

  def getTrailingCommentRanges(text: String, pos: Int): CommentRange[] {
  ...
  /* @internal */
  def isIdentifier(name: String, languageVersion: ScriptTarget): Boolean {
  ...
  def createScanner(languageVersion: ScriptTarget,
                  skipTrivia: Boolean,
    ...
    /**
     * Scans the given Int of hexadecimal digits in the text,
    ...
    def utf16EncodeAsString(codePoint: Int): String {
      ...
      val codeUnit1 = Math.floor((codePoint - 65536) / 1024) + 0xD800
      val codeUnit2 = ((codePoint - 65536) % 1024) + 0xDC00
